...
module KeyboardInput =
    let diagnostic severity code message modeId commandId keyPositionId =
        { Severity = severity
          Code = code
          Message = message
          ModeId = modeId
    ...
    let error code message modeId commandId keyPositionId =
    ...
    let renderLayoutStateAt position runtime =
        let x, y = position
        let view = layoutState runtime
        let modes = view.ActiveModeStack |> List.map (fun frame -> frame.ModeId) |> String.concat " > "
        let state = view.ActiveModeStack |> List.tryPick _.State |> Option.defaultValue "none"
        let held =
            view.HeldModes
            |> List.map _.ModeId
            |> String.concat ", "
        ...
        let keys =
            view.ActiveLabels
            |> Map.toList
            |> List.map (fun (key, label) -> $"{key}={label}")
            |> String.concat "  "

        let textLines =
            [ $"layout: {view.ActiveLayout.DisplayName} ({view.ActiveLayout.Id})"
              $"modes: {modes}"
              $"state: {state}"
              $"held: {held}"
              $"keys: {keys}" ]
        ...
        let panelHeight = padding * 2.0 + lineHeight * float textLines.Length

        Scene.group [
            Scene.rectangle (x, y, panelWidth, panelHeight) (Colors.rgba 10uy 14uy 20uy 220uy)
            for index, line in textLines |> List.indexed do
                textAt (x + padding) (y + padding + 12.0 + float index * lineHeight) line
        ]
