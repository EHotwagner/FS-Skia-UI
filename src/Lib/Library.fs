...
module VulkanHost =
    ...
    let attachWindowEventMapping program (window: IWindow) dispatch =
        ...
        let closingHandler =
            Action(fun () -> dispatchViewerEvent program dispatch CloseRequested)
    ...
    let drawScene scene (canvas: SKCanvas) =
        let rec drawNode node =
            ...
            | Text((x, y), text, color) ->
                use paint = new SKPaint()
                use font = new SKFont(SKTypeface.Default, 24.0f)
    ...
    let run program =
        ...
        let shouldRunEventLoop () =
            not shutdownRequested
            && ((activeSubscriptions |> List.isEmpty |> not)
                || (program.EventMapper Loaded |> Option.isSome))

        let runEventLoop (createdWindow: IWindow) =
            if shouldRunEventLoop () then
                trace program.Configuration "entering Silk.NET event loop"

                while not createdWindow.IsClosing && not shutdownRequested do
                    createdWindow.ContinueEvents()
        ...
        let execute () =
            try
                ...
                bind (createWindow program.Configuration) (fun createdWindow ->
                    ...
                    windowEventMapping <- Some(attachWindowEventMapping program createdWindow dispatch)
