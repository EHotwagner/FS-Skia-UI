...
/// A typed routing rule: glob path patterns (the single source for both the `Matches`
/// predicate and the generated contract's `paths:` list), the Tier they select, the
/// required gates (typed Targets.Target — mistype ⇒ compile error), the artifacts
/// `--enforce` checks, and the rendered `timeout_class`/`failure_owner`.
type RoutingRule =
    { Id: string
      Paths: string list
      Matches: Diff -> bool
