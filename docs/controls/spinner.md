---
title: Spinner
category: Controls
categoryindex: 8
index: 49
description: Indeterminate progress indicator.
---

<!-- BEGIN GENERATED: catalog-docs/spinner -->
# Spinner

- **Category:** feedback
- **Purpose:** Indeterminate progress indicator.
- **API reference:** [FS.Skia.UI.Controls.Spinner](../reference/fs-skia-ui-controls-spinner.html)

[← Back to the controls catalog](catalog.html)
<!-- END GENERATED: catalog-docs/spinner -->

## Overview

The **Spinner** control is a `feedback`-category control in the FS.Skia.UI control suite.
Indeterminate progress indicator. It has no required attributes — every attribute is optional. It raises no events (it is a non-interactive, display-only control). Its accessibility role is `Progress`.

Build it through the `FS.Skia.UI.Controls.Spinner` module — the typed Props/MVU front door is the
preferred authoring surface (see
[Typed control front door](../controls-design/typed-front-door.html)). Every
attribute is owned by the model; the control renders against the active `Theme`,
so its colours, sizing, and density follow the design tokens (see
[Controls in the Spec Kit workflow](spec-kit-workflow.html)).

## Usage example

A runnable example that exercises this control lives in the controls gallery
sample, `samples/ControlsGallery/Program.fs`. It is compiled as part of the build,
so the example stays in lock-step with the public API.

## Preview

![Spinner render-only preview](../img/controls/spinner.png)

A deterministic **render-only** preview of **Spinner** (320×160), produced through the
render-only evidence path (`Widget.render` → `SkiaViewer.captureScreenshotEvidence`,
`ViewerRenderTargetPng`) and validated decodable / non-1×1 / non-trivial via
`Testing.readPngArtifact`. It shows the control rendered against the default
`DesignTokens.Light` theme.

[← Back to the controls catalog](catalog.html)
