...
<!-- BEGIN GENERATED: catalog-docs/split-view -->
# Split View

- **Category:** layout
- **Purpose:** Resizable two-region layout.
...
<!-- END GENERATED: catalog-docs/split-view -->

## Overview

The **Split View** control is a `layout`-category control in the FS.Skia.UI control suite.
Resizable two-region layout. It requires the `children` attribute. It raises `onChanged`. Its accessibility role is `StaticText`.
...
![Split View render-only preview](../img/controls/split-view.png)

A deterministic **render-only** preview of **Split View** (320×160), produced through the
