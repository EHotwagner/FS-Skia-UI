...
categoryindex: 8
index: 45
description: Modal content region.
---

<!-- BEGIN GENERATED: catalog-docs/dialog -->
# Dialog

- **Category:** overlay
- **Purpose:** Modal content region.
- **API reference:** [FS.Skia.UI.Controls.Dialog](../reference/fs-skia-ui-controls-dialog.html)

[← Back to the controls catalog](catalog.html)
<!-- END GENERATED: catalog-docs/dialog -->

## Overview

The **Dialog** control is a `overlay`-category control in the FS.Skia.UI control suite.
Modal content region. It requires the `children` attribute. It raises `onSelected`. Its accessibility role is `Dialog`.

Build it through the `FS.Skia.UI.Controls.Dialog` module — the typed Props/MVU front door is the
...
![Dialog render-only preview](../img/controls/dialog.png)

A deterministic **render-only** preview of **Dialog** (320×160), produced through the
