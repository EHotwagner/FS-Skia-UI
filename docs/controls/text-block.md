...
categoryindex: 8
index: 10
description: Static model-owned text display.
---

<!-- BEGIN GENERATED: catalog-docs/text-block -->
# Text Block

- **Category:** display
- **Purpose:** Static model-owned text display.
- **API reference:** [FS.Skia.UI.Controls.TextBlock](../reference/fs-skia-ui-controls-textblock.html)

[← Back to the controls catalog](catalog.html)
<!-- END GENERATED: catalog-docs/text-block -->

## Overview

The **Text Block** control is a `display`-category control in the FS.Skia.UI control suite.
Static model-owned text display. It requires the `text` attribute. It raises no events (it is a non-interactive, display-only control). Its accessibility role is `StaticText`.

Build it through the `FS.Skia.UI.Controls.TextBlock` module — the typed Props/MVU front door is the
...
![Text Block render-only preview](../img/controls/text-block.png)

A deterministic **render-only** preview of **Text Block** (320×160), produced through the
