...
<!-- BEGIN GENERATED: catalog-docs/pie-chart -->
# Pie Chart

- **Category:** chart
- **Purpose:** Controls-owned part-to-whole visualization.
- **API reference:** [FS.Skia.UI.Controls.PieChart](../reference/fs-skia-ui-controls-piechart.html)

[← Back to the controls catalog](catalog.html)
<!-- END GENERATED: catalog-docs/pie-chart -->

## Overview

The **Pie Chart** control is a `chart`-category control in the FS.Skia.UI control suite.
Controls-owned part-to-whole visualization. It requires the `values` attribute. It raises `onSelected`. Its accessibility role is `Chart`.

Build it through the `FS.Skia.UI.Controls.PieChart` module — the typed Props/MVU front door is the
...
![Pie Chart render-only preview](../img/controls/pie-chart.png)

A deterministic **render-only** preview of **Pie Chart** (320×160), produced through the
