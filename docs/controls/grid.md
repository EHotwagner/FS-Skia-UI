...
<!-- BEGIN GENERATED: catalog-docs/grid -->
# Grid

- **Category:** layout
- **Purpose:** Structured child composition.
- **API reference:** [FS.Skia.UI.Controls.Grid](../reference/fs-skia-ui-controls-grid.html)

[← Back to the controls catalog](catalog.html)
<!-- END GENERATED: catalog-docs/grid -->

## Overview

The **Grid** control is a `layout`-category control in the FS.Skia.UI control suite.
Structured child composition. It requires the `children` attribute. It raises no events (it is a non-interactive, display-only control). Its accessibility role is `StaticText`.

Build it through the `FS.Skia.UI.Controls.Grid` module — the typed Props/MVU front door is the
...
![Grid render-only preview](../img/controls/grid.png)

A deterministic **render-only** preview of **Grid** (320×160), produced through the
