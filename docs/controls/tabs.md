...
<!-- BEGIN GENERATED: catalog-docs/tabs -->
# Tabs

- **Category:** navigation
- **Purpose:** Model-owned active page selection.
- **API reference:** [FS.Skia.UI.Controls.Tabs](../reference/fs-skia-ui-controls-tabs.html)

[← Back to the controls catalog](catalog.html)
<!-- END GENERATED: catalog-docs/tabs -->

## Overview

The **Tabs** control is a `navigation`-category control in the FS.Skia.UI control suite.
Model-owned active page selection. It requires the `items` attribute. It raises `onChanged`. Its accessibility role is `Tab`.

Build it through the `FS.Skia.UI.Controls.Tabs` module — the typed Props/MVU front door is the
...
![Tabs render-only preview](../img/controls/tabs.png)

A deterministic **render-only** preview of **Tabs** (320×160), produced through the
