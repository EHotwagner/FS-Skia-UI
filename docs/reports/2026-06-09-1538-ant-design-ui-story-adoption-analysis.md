...
- Date: 2026-06-09 (addendum 2026-06-12: "Agent Infrastructure Strategy" section added)
...
*Added 2026-06-12. Ant Design is a large, information-dense, durable knowledge area.
This section answers a practical question the rest of the report assumes but does not
state: how should the repo's agent infrastructure (skills, reference data, subagents,
workflows, memory, gates) be used to incorporate it? The decision rule is not "how big
is the topic" but:*

> **Match the mechanism to the lifetime and reuse of the artifact.** Durable,
> reused-every-feature knowledge belongs in skills + reference data. One-time or
> iterative acquisition/transformation belongs in subagents/workflows. Tiny decisions
> belong in memory. Enforcement belongs in gates over a single source.

### Mechanism selection

| Mechanism | Lifetime | Loaded into agent context? | Best Ant-adoption use |
|-----------|----------|----------------------------|------------------------|
| Skill (`.agents/skills/*`) | durable, versioned, governed, mirrored to `.claude` | yes, on demand | the distilled, procedural front door: token taxonomy, the palette algorithm's contract, design principles, when-to-use and translation rules |
| Reference data file (JSON/MD the skill points to) | durable | no (read by tools/agents on demand) | the bulk: full Ant palettes, per-component specs, spacing/motion/elevation tables, the `llms-semantic`/`llms-full` distillation |
| Subagent (`Agent` / `deep-research`) | ephemeral (one session), own context window | isolated | acquiring and transforming: harvesting Ant docs via MCP/`llms-full.txt`, extracting the color math, auditing a screen, one-off migrations |
| Workflow (deterministic fan-out) | ephemeral | per-agent | batch transform/audit: generate+verify the full token set across light/dark, or port/audit all controls against Ant specs in parallel |
| Memory | durable, tiny | yes (index) | the decision record ("adopted Ant v5/v6 seed->map->alias; policy selector is `wcag`/`ant`"), not bulk knowledge |
| Governance gate | durable enforcement | n/a | keep generated Ant tokens correct over time (`DesignTokenDrift`, policy-backed color check) |

### Layered architecture (not "just skills")

1. **Knowledge layer = a thin skill family + fat reference files.** Do not pour Ant
   Design into one mega-skill; skills load into agent context and a 50KB skill is a tax
   on every task. Use the repo's existing thin-skill-over-single-source pattern
   (`fs-skia-design-tokens` already points at `design-tokens.tokens.json`). Refinement of
   the "Draft Local Skill" above: keep `fs-skia-ant-design` as the **procedural front
   door** (scope, source priority, translation rules, checks), and move the dense content
   into reference data files it points to:
   - `docs/reference/ant-design/color-system.md` (+ a token data file) — seed/map/alias
     taxonomy and the palette derivation.
   - `docs/reference/ant-design/principles.md` — values, spacing, motion, elevation, icon,
     density tables.
   - `docs/reference/ant-design/patterns.md` — per-component and per-page recipes.
   Optionally split a second focused skill (`fs-skia-ant-color` vs `fs-skia-ant-patterns`)
   only if the single skill's front door grows past a screen.

2. **Acquisition/maintenance layer = subagents + workflows (ephemeral).** The skills are
   the *output*; agents are *how it is produced and kept current*. The official feeds this
   report already lists are the inputs: the Ant **MCP server** (`antd mcp`) and
   `llms-full.txt`/`llms-semantic.md` are ideal subagent fuel because an isolated context
   window can digest them without polluting the main thread. Use `deep-research` for the
   color algorithm and contrast tables; use a `Workflow` for batch token generation/verify
   and for control/page audits.

3. **Enforcement = the token single source + gates.** Make Ant tokens *generated* from the
   seed via the palette algorithm, never hand-copied, so they cannot drift and always pass
   the selected color policy. This is the `DesignTokenGen` + `DesignTokenDrift` +
   policy-backed color-check path the report already describes.

4. **Process = treat adoption as a Spec Kit feature/constitution change**, run through
   specify -> plan -> tasks -> implement, so the skill family, reference files, policy
...
one seed via HSV hue/saturation/brightness curves, plus the dark-mode blend) as a **pure
F# function feeding the DTCG generator**. Ant's three-tier seed -> map -> alias model maps
exactly onto DTCG and the local token pipeline, so this one function yields Ant-correct,
drift-checked, contrast-gated palettes for any brand color. Everything else (principles,
per-component parity, generated profiles) layers on top of it. The MIT license on Ant's
color package makes reimplementation (with attribution) clean.

### Anti-patterns to avoid

- Dumping Ant prose into skills (context bloat; the durable unit is the *algorithm + the
  single-source data*, with the skill as a thin front door).
- Hand-maintaining bulk reference content that an agent could regenerate from the MCP/LLM
  feeds.
- Using ad-hoc subagents to produce knowledge and then never writing it into a durable
  skill/reference/token file (ephemeral work that evaporates).
- Hand-syncing `.agents` and `.claude` (let the generator own the mirror).
