...
performance-critical routine. "Pipelines over mutation" is the default, not a
prohibition. Disclose the reason at the use site with a one-line comment (e.g.
`// mutable: hot path`) so a reader doesn't waste effort "fixing" it.
...
tricks, so a simplicity bias keeps code legible to future maintainers. Dogmatic
immutability — recursion gymnastics in place of an obvious loop, or a
fold-with-state where a `mutable` would read straight through — is itself the
cleverness this principle discourages.
...
placeholders, canned responses, or any test that exercises only literal data — MAY be used
...
the relevant test-framework skip attribute, or the test framework's skip mechanism) with written
