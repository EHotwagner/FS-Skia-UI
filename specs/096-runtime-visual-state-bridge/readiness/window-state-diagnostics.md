...
- diagnostic-class=app-lifecycle — no persistent app lifecycle is started; the deterministic
  precedence / bridge / live-retained / responds-proof suites run to completion and exit.
...
made — there is simply no window in scope. The bridge is exercised through the pure `deriveVisualState`
projection, the pure `applyRuntimeVisualState` tree walk, and the live retained render-step path
(`RetainedRender.init`/`step`); the responds-proof uses the production render path (off-window), not a
windowed host.
