...
- option=backend — backend=not-applicable (no windowed backend selected; the proofs run through the
  pure `deriveVisualState` projection, the pure `applyRuntimeVisualState` bridge, and the off-window
  `RetainedRender` / `Control.renderTree` paths); status=not-applicable observed=false
