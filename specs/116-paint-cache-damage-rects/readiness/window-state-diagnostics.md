# window-state-diagnostics — applicability (feature 116, T002)

Every diagnostic-class is not-applicable for feature 116 (no new window is created; the existing
interactive host launch contract is unchanged — the change is an internal damage-set + bounded
picture-cache + advisory diagnostic + additive `FrameMetrics` fields, with no edit to the
`runInteractiveApp` launch seam).
