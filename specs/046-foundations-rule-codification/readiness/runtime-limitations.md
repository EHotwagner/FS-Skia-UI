...
This feature is **build-tooling / governance only** and changes no runtime path; the
runtime-limitation statements are reproduced here for completeness and remain unchanged.
...
No runtime/visual/screenshot evidence is in scope (SC-009); the product `src/**` surface
and all visual paths are untouched.
