# Generated Validation (feature 110)
...
`.fsi` change** to `FrameMetrics` in `FS.Skia.UI.Controls.Elmish` (the new `FullRenderFallbackCount`
field), `Route` **escalates** to the controls-public-surface (maintainer-verify) route. Only the gates
...
Feature 110 changes only `FS.Skia.UI.Controls.Elmish` (the live pointer route + the `FrameMetrics`
field + the `Perf.runScript` corpus driver) plus the internal `FS.Skia.UI.Controls` retained surface
(the new `authoredControlIds` lookup) and the test/evidence corpus + baselines. The `dotnet new
fs-skia-ui` template pins the currently-published package versions; those versions resolve cleanly
...
the additive field and needs no edit. The 110 public surface reaches generated projects only after the
squash-merge version bump (the separate template-pin track), exactly as every prior controls-public-
surface feature. `generated-tests-exist=false` / `generated-tests-ran=not-applicable` because 110
introduces no new generated-project test; `authoritative=false` because `GeneratedProductCheck` is not
the authoritative signal for this change (the authoritative signal is the escalated controls suite +
...
110 API and `GeneratedProductCheck` is green. Hence `authoritative=false` for any pre-bump run.
