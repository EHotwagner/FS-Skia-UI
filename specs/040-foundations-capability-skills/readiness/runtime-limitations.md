...
**None to disclose.** It edits no `src/**` source, ships nothing in any
generated product, and adds no persistent host process. The only executable
behaviour added is two build-tooling FAKE gates: an in-process SHA-256 compare
and a `dotnet build` of an isolated examples project — build tooling, not
product runtime. No `FSharp.Compiler.*` is introduced (FR-008).
