# close-reason-separation — feature 118 (US1)

close-reason=evidence-self-close
user-close-observed=false
evidence-close-observed=true

The feature-118 live-host run closes for a single reason: the bounded evidence frame count was
reached (the host requests `Shutdown` at frame 40). This is an evidence self-close
...
close → `CloseRequested`) contract is unchanged by this feature.
