// CONTRACT SKETCH — build-side DTCG parser/generator (Phase 1).
// Final home: build/Governance/DesignTokenGen.fsi  (FS.Skia.UI.Build, net10.0; build-tooling scope —
// NOT a tracked runtime surface baseline, same status as CatalogGen.fsi / SkillTreeGen.fsi).
// Mirrors build/Governance/CatalogGen.fsi exactly (TypedCatalogFact/RegionStatus/CatalogCurrency,
// catalog*Rel, render*, splice*, currency/isCurrent/currencyDrift). Pure over in-memory text; the
// only filesystem read/write stays at the Engine/Interpret.fs edge (Principle IV).
//
// Difference from CatalogGen: the single source is the checked-in DTCG JSON document (parsed here),
// not a compiled F# fact value — so this surface adds `parse` and the generated module is whole-file
// (D2, D3). currencyDrift still names the divergent token per-token (FR-004).
...
/// Pure: hex color -> `Colors.rgba R G B A`; dimension/number -> float w/ decimal point;
/// fontFamily null -> `None`, concrete -> `Some "<name>"` (D5).
val renderValue: fact: DesignTokenFact -> string

/// Render the whole generated module text from the fact table (whole-file generation, D3),
/// including the `GENERATED — do not edit` banner and the regenerate command. Pure.
