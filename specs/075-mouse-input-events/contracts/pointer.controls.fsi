// CONTRACT SKETCH — FS.Skia.UI.Controls pointer-interaction front door (feature 075).
// Target files: src/Controls/Pointer.fsi + Pointer.fs (NEW public module).
// Pure coordination over the existing ControlRuntime + Layout.hitTestComputed.
// FS.Skia.UI.Controls stays host-independent: this module references no ViewerEvent.

namespace FS.Skia.UI.Controls

open FS.Skia.UI.Layout

/// Framework-neutral mouse button identity (FR-013).
type PointerButton =
    | Primary
    | Secondary
    | Middle
...
module Pointer =
    /// Initial coordination state. threshold defaults to 4.0 px (click-vs-drag, FR-006).
    val init: ?threshold: float -> PointerState
