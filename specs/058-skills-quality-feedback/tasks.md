...
- [-] T027 [P] [US3] [skillist: fs-skia-template-update] Add failing-first template-generation tests: `--feedback false` byte-identity vs current output, `--feedback true` presence of the three prompts + `feedback/` wiring, and an aborted phase writing nothing — **SKIPPED (rationale)**: `--feedback false` byte-identity is guaranteed by construction (all feedback content lives in `feedback == true` conditional `sources`; the symbol itself emits no output), and the empirical generation-diff is covered by the `TemplateCheck`/`GeneratedProductCheck` run (T031), not yet executed in this session. A dedicated failing-first generation test is deferred with the T031 pipeline run. **Update
...
- [X] T029 [US3] [skillist: []] Author the new `fs-skia-feedback-capture` command skill (the three exact prompts with `{phase}` substitution + the record schema) in `.agents/skills/`, raised to the rubric, shipped in the template feedback branch — authored at `template/feedback/skill/SKILL.md` (NOT repo `.agents/skills/`, which ships unconditionally and would break SC-006 byte-identity); shipped via the `feedback == true` source; passes `SkillQualityCheck` (now in-scope)
...
- [X] T032 [US3] [skillist: fs-skia-template-update] Capture `readiness/template-feedback-false.md` (default byte-identity, SC-006) and `readiness/template-feedback-true.md` (three prompts wiring + `feedback/` folder, SC-005) — **empirically verified** (2026-06-03): `diff -rq --exclude=.git` of two identically-named generations (`--feedback false` vs `true`) shows exactly the three conditional feedback sources added and **zero other diff** (SC-006); the `true` project carries all six `after_*` hooks + the three `{phase}` prompts (SC-005).
