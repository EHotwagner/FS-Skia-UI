...
The durable `runInteractiveApp` launch self-closed on the host's own `Tick -> CloseWindow`
request, which the runner classified as `close-reason=AppRequestedClose`
...
first frames presented), which is distinct from both a user close and an
`EvidenceRequestedClose`. Source: `readiness/logs/interactive-launch.txt`.
